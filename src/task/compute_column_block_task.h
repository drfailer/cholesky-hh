...
template<typename T>
using CCBTaskInputType = std::pair<std::shared_ptr<MatrixBlockData<T, Block>>, std::shared_ptr<MatrixBlockData<T, Block>>>;
...
class ComputeColumnBlockTask : public hh::AbstractTask<CCBTaskInNb, CCBTaskIn, CCBTaskOut > {
 public:
  ComputeColumnBlockTask(size_t nbThreads) : hh::AbstractTask<CCBTaskInNb, CCBTaskIn, CCBTaskOut >(
  ...
  void execute(std::shared_ptr<CCBTaskInputType<T>> blocks) override {
    ...
    cblas_dtrsm(CblasRowMajor, CblasLeft, CblasUpper,
                ...
                diagBlock->get(), diagBlock->matrixWidth(), colBlock->get(), colBlock->matrixWidth());
  }
